  ...
  return (
    <section className='w-full min-h-full px-auto bg-lightWhiteBG font-inter'>
      <div className='flex flex-col gap-4 relative'>
        {/* Header */}
        <div className='w-full h-60 flex flex-col justify-between bg-lightVeryPaleBlueTop gap-1 px-6 py-10 pb-14 rounded-bl-[30px] rounded-br-[30px]'>
          <div className='flex flex-col gap-2'>
            <h1 className='font-extrabold text-[26px] leading-6 capitalize'>Social media dashboard</h1>
            <p className='leading-normal font-semibold text-lightDarkGrayishBlueText'>Total Follower 23,452</p>
          </div>
          <hr/>
          <div className='flex justify-between items-center'>
            <span className='font-bold text-lightDarkGrayishBlueText '>Dark Mode</span>
        ...
        <div className='w-full h-auto absolute top-52 px-6 flex flex-col gap-3'>
          {socialMedia?.map(({platform, follower, dailyFollower, icon, riseUp}) => (
            <div className={`w-full rounded-xl ${platform === "instagram"
        ...
        <div className='w-full h-auto py-8 flex flex-col gap-10'>
          ...
          <div className='flex flex-col gap-4'>
            ...
            { socialOverview?.map(({name, icon, number, rise, riseNumber }) => (
              <div className='flex flex-col gap-8 bg-lightGrayishBlueCard p-6 rounded-2xl'>
                <div className='flex justify-between items-center'>
